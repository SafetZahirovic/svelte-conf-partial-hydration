		...
		async resolveId(id, importer, options) {
			...
			if (crownMatch) {
				...
				if (mod === "navigation") {
					return await this.resolve(
						options.ssr ? "@crown/jewels/server/navigation.ts" : "@crown/jewels/client/navigation.ts",
						importer,
						{ skipSelf: true }
					);
				} else if (mod === "stores") {
					return await this.resolve(
						options.ssr ? "@crown/jewels/server/stores/index.ts" : "@crown/jewels/client/stores/index.ts",
						importer,
						{ skipSelf: true }
					);
				} else if (mod === "env") {
					return await this.resolve(
						options.ssr ? "@crown/jewels/server/env.ts" : "@crown/jewels/client/env.ts"
					);
				}
			...
			if (options.ssr) {
				...
				if (matches) {
					...
					if (resolution) {
						const path = resolution.id.replace(normalizePath(resolve(process.cwd())), "");

						if (server) {
							const crownComponents = server.moduleGraph.idToModuleMap.get(crownComponentsLoad);
		...
		async load(id, options) {
			if (id === crownComponentsLoad) {
				...
				if (dev || (options && options.ssr)) {
					...
					const manifest = require(resolve("./dist/server/dynamic-components.json")) as string[];
			...
			if (options && options.ssr) {
				...
				if (matches) {
					...
					return generatePatchedComponent(
						realId,
						svelteRequest?.normalizedFilename.slice(1).replace(`${mode}:`, "") || realId,
						mode
					);
				}
			}
