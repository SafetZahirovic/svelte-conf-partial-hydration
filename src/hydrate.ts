...
export function lazy() {
	...
	document
		...
		.forEach((el) => el.querySelectorAll("*").forEach((childEl) => lazyObserver.observe(childEl)));
}

export function hydrate() {
	// Hydrates a server side rendered page by hydrating Svelte components where they are rendered
	document.querySelectorAll("crown-component[data-mode=hydrate]").forEach((el) => attach(el as HTMLElement, true));
}

export async function attach(el: HTMLElement, hydration: boolean = true) {
...
export function update(current: HTMLElement, next: HTMLElement) {
	...
	promise.then((instance) => instance.$set(JSON.parse(decodeURIComponent(next.dataset.props || ""))));
}
